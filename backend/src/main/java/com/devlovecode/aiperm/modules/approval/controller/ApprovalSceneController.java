...
public class ApprovalSceneController {
	...
	@GetMapping
	public R<PageResult<ApprovalSceneVO>> page(
			@Validated({ Default.class, Views.Query.class }) ApprovalSceneDTO dto) {
		return R.ok(approvalSceneService.queryPage(dto));
	}

	@Operation(summary = "查询审批处理器列表")
	@SaCheckPermission("system:approval-scene:list")
	@GetMapping("/handlers")
	public R<List<ApprovalHandlerVO>> handlers() {
		return R.ok(approvalSceneService.listHandlers());
	}

	@Operation(summary = "查询审批场景详情")
	@SaCheckPermission("system:approval-scene:list")
	@GetMapping("/{id}")
	public R<ApprovalSceneVO> detail(@PathVariable Long id) {
	...
	@PostMapping
	public R<Long> create(@RequestBody @Validated({ Default.class, Views.Create.class }) ApprovalSceneDTO dto) {
		return R.ok(approvalSceneService.create(dto));
	}
	...
	public R<Void> update(@PathVariable Long id,
			@RequestBody @Validated({ Default.class, Views.Update.class }) ApprovalSceneDTO dto) {
