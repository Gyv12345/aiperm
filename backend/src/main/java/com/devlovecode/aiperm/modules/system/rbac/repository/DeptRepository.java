...
public interface DeptRepository extends BaseJpaRepository<SysDept> {
	 ...
	 * 检查是否有子部门
	 */
	@Query("SELECT COUNT(d) > 0 FROM SysDept d WHERE d.parentId = :parentId")
	boolean hasChildren(@Param("parentId") Long parentId);

	/**
	 * 检查部门名称是否重复（同父级下）
	 */
	@Query("SELECT COUNT(d) > 0 FROM SysDept d WHERE d.deptName = :deptName AND d.parentId = :parentId")
	boolean existsByDeptNameAndParentId(@Param("deptName") String deptName, @Param("parentId") Long parentId);

	/**
	 * 检查部门名称是否重复（同父级下，排除指定ID）
	 */
	@Query("SELECT COUNT(d) > 0 FROM SysDept d WHERE d.deptName = :deptName AND d.parentId = :parentId AND d.id != :id")
	...
	SysDept findByDeptNameAndDeleted(String deptName, Integer deleted);
