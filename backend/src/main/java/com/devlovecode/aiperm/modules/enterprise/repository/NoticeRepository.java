...
public interface NoticeRepository extends BaseJpaRepository<SysNotice> {
    ...
    int publish(@Param("id") Long id, @Param("updateBy") String updateBy, @Param("publishTime") LocalDateTime publishTime, @Param("updateTime") LocalDateTime updateTime);
    ...
    int withdraw(@Param("id") Long id, @Param("updateBy") String updateBy, @Param("updateTime") LocalDateTime updateTime);

    /**
     * 查询已发布的公告列表
     */
    @Query("SELECT n FROM SysNotice n WHERE n.status = 1 AND n.deleted = 0 " +
