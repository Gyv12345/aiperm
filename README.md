...
> 我在实际业务中做过一个基于 NewAPI 的精简代理版：只保留模型请求转发能力，避免前端暴露平台 API Key；请求前由 Java 服务校验当前登录用户是否具备调用资格和额度。这个开源项目是在该实践之上抽象出的通用版本。对于
> Agent 场景，可采用前端 BYOK 模式，API Key 仅保存在用户本地，有则使用、无则不启用。
